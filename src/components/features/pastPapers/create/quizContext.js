...
import { pastPaperApi, questionApi } from "../../../../services/Constant";
...
import { setQuestionList } from "../../../../ducks/pastPaperSlice";
import ModalComp from "../../../common/modal";
import { LoadingButton } from "@mui/lab";
import { useNavigate, useParams } from "react-router-dom";
import { useUpdatePastPaperMutation } from "../../../../services/apis/pastPapersApi";
...
function QuizContext() {
  ...
  const [selectedSectionId, setSelectedSectionId] = useState(
    pastPaperDetail?.sections[0]?.section._id
  );
  const [updatePastPaper, { isLoading: updateLoading }] =useUpdatePastPaperMutation();
  ...
  const handleSelectAll = () => {
    ...
    const updatedSection = {
      ...pastPaperDetail.sections[sectionIndex],
      questions: newChecked,
    };
    const updatedList = [...pastPaperDetail.sections];
...
const fetchList = () => {
  ...
  const findActiveSection = useMemo(() => {
    const updatedQuestionsList = [...pastPaperDetail.sections];
    console.log(
  ...
  }, [selectedSectionId, pastPaperDetail]);

  console.log(selectAllChecked, "2222");

  const handleToggle = (value) => () => {
    const sectionIndex = pastPaperDetail.sections.findIndex(
      ({ section }) => section._id === selectedSectionId
    );

    if (sectionIndex !== -1) {
      const questions = [...pastPaperDetail.sections[sectionIndex].questions];
    ...
    if (params.paperId) {
      try {
        ...
        const response = await updatePastPaper({
          endpoint: `${pastPaperApi.endPoint}/${params.paperId}`,
        ...
        if (response && response?.data?.success) {
          ...
          message.error("Some error occured to Add Past-Paper!", 2.5);
  ...
  console.log(
    "🚀 ~ QuizContext ~ sections.length === findActiveSection :",
    pastPaperDetail.sections.length,
    findActiveSection
  );
    ...
    <div className="bg-white rounded-md mt-3 h-[80vh]  p-2 ">
      <div className="flex">
                        ...
                        <ListItem
                          ...
                          secondaryAction={
                            <Checkbox
                              ...
                              checked={pastPaperDetail.sections[
                                findActiveSection
                              ].questions.some(
                                (question) => question.qid === value.qid
                              )}
        ...
        <section className="basis-[80%] p-2  ">
          <Stack
            sx={{ overflowX: "scroll", width: "max-content", mb: 1 }}
            direction={"row"}
            spacing={3}
          >
            {pastPaperDetail?.sections?.map(({ questions, section }, ind) => (
              <Badge
                size="large"
                style={{
          ...
          <div className="h-[70vh] overflow-scroll scrollbar-hide flex flex-col gap-3 justify-start my-2">
            {pastPaperDetail.sections?.[findActiveSection]?.questions?.length >
              0 &&
              pastPaperDetail.sections[findActiveSection]?.questions.map(
                (value, ind) => (
                  <>
      ...
      {pastPaperDetail.sections.length - 1 === findActiveSection && (
        <CustomButton
          size="small"
          onClick={handleNext}
          sx={{
        ...
        <div className="w-[627px] p-1 flex flex-col gap-2">
          ...
          <Stack direction="row" justifyContent="flex-end" spacing={2} my={2}>
            ...
            <LoadingButton
              ...
              onClick={() => navigate(`/main/exam/pastPapers/preview`)}
              loadingIndicator={
