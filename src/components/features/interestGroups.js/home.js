...
import {
  ...
  useGetGroupListQuery as useGetInterestListQuery,
...
function CreateGroupModal() {
  ...
  const [createGroup, { isLoading, isFetching,isError }] = useCreateGroupMutation();
  ...
  const {
    interests,
    isLoading: interestLoad,
    isError: interestError,
    isFetching: interestFetch,
  } = useGetInterestListQuery(`/admin/v1/interest/list`, {
    refetchOnMountOrArgChange: true,
    refetchOnReconnect: true,
    selectFromResult: ({ data, isLoading, isError }) => {
      if (!isLoading && !isError && data) {
        return { interests: data?.data.interest };
      }
      return [];
    },
  });
console.log("🚀 ~ CreateGroupModal ~ interests:", interests);
console.log("🚀 ~ CreateGroupModal ~ value:", value);
  return (
    <div>
      <Button
        variant="contained"
          ...
          <div className="w-[627px] mt-7 p-2 flex flex-col gap-4">
            ...
            <div>
              ...
              <MultiSelectOutlined
                ...
                onChange={handleChange}
                data={interests}
                loading={interestFetch || interestLoad}
                error={interestError}
                value={value.interestId}
              />
              <small className="text-red-700">
                {errors.interest && "Please select Host id"}
              </small>
            </div>
            ...
            {value.interestId && (
              <div>
                ...
                <MultiSelectOutlined
                  name="subInterestId"
                  size="small"
                  labelId="subInterestId"
                  margin="none"
                  onChange={handleChange}
                  data={
                    interests.find((val) => val._id === value.interestId)?.items
                  }
                  loading={interestFetch || interestLoad}
                  error={interestError}
                  value={value.subInterestId}
                />
                <small className="text-red-700">
                  {errors.interest && "Please select Host id"}
                </small>
...
function Home() {
  ...
  const {
    ...
    `exams/v1/group/interest/list?page=1&count=${count}&search=${searchTerm}`,
    {
      refetchOnMountOrArgChange: true,
      refetchOnReconnect: true,
    }
  ...
  return (
    <div className="relative bg-white rounded-md">
      <Header content={`Interest Groups`} />
