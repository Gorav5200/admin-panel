...
import { entityApi, entityTypeApi, subjectApi } from "../../../../services/Constant";
  ...
  const handleCreate = async () => {
    try {
      const response = await addEntity({
        endpoint: `${entityTypeApi.endPoint}`,
        data: values,
      });
  ...
  const handleUpdate = async (id) => {
    try {
      const response = await updateEntity({
        endpoint: `${entityTypeApi.endPoint}/${id}`,
        updateEntity: values,
      });
      if (response.data && response.data.success) {
      ...
      <Box className="modalRoot max-h-[60vh] overflow-scroll">
        <header className="ps-2 flex justify-between items-center bg-medGrey rounded-t-md">
          <h4 className="text-xl font-inter font-semibold text-gray-700">
            {compType === "create" ? "Create Entity Type" : "Edit Entity Type"}
          </h4>
          <IconButton
            onClick={() => {
              handleClose();
        ...
        <div className="w-[627px] mt-7 p-2 flex flex-col gap-4">
          <div className="flex justify-between items-start gap-2">
            ...
            <div>
              <SingleImageUpload
                endpoint={`${entityTypeApi.endPoint}/upload/image`}
          ...
          <div>
            ...
            <Select
              ...
              value={values.business_subcategory_id}
              defaultValue={values.business_subcategory_id}
              required
              onChange={handleChange}
              input={<OutlinedInput />}
              MenuProps={MenuProps}
