...
function ClassList() {
  ...
  const {
    ...
    `${classApi.endPoint}/entityType/${params.examId}?page=${
      page + 1
    }&count=${rowsPerPage}${searchTerm ? `&search=${searchTerm}` : ""}`,
    {
      refetchOnMountOrArgChange: true,
    }
  );

  console.log("🚀 ~ ClassList ~ classList:", classList);
  ...
  return (
    <>
      ...
      <div className="flex flex-wrap justify-start gap-8 overflow-scroll h-[75vh] my-2 items-start">
        ...
        ) : isSuccess && classList?.data?.classList?.length === 0 ? (
          <div className="m-auto">
            <Empty Description={<p className="font-inter">No Data Found</p>} />
          </div>
        ) : (
          <div className="flex flex-wrap justify-start gap-8 overflow-scroll h-[75vh] pb-10 p-3 items-start w-full">
            <TableContainer component={Paper}>
              <Table>
                ...
                <TableBody>
                  {classList?.data?.classList?.map((row) => (
                    <TableRow
                      key={row._id}
                      sx={{ cursor: "pointer" }}
                      onClick={() =>
                        navigate(
                          `/main/entity/${params.examId}/class/${row._id}/detail`
                        )
                      }
                    >
