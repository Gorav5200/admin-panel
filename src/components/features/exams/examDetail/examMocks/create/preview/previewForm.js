...
import PublishMock from "./publishMock";
import { useNavigate, useParams } from "react-router-dom";
import { checkQuestionType } from "../../../../../../../services/common";
const PreviewForm = () => {
  const navigate = useNavigate();
  const params = useParams();
  const { sections } = useSelector((state) => state.mock.mockDetail);
      ...
      <div className="flex gap-2 h-[70vh] ">
        <div className="basis-[25%] bg-white rounded-md overflow-scroll p-2 text-black ">
          ...
          <CustomButton
            ...
            sx={{ float: "right", m: 2 }}
            onClick={() => navigate(`/main/exam/${params.examId}/mocks/create`)}
          >
            Add Import
          </CustomButton>
        </div>
        ...
        <div className="basis-[50%] info bg-white rounded-md overflow-scroll scrollbar-hide ">
          <header className="p-2 pt-3 sticky top-0 bg-white z-50">
                  ...
                  selectedQus?.options?.map((item, index) => (
                    <Card
                      ...
                      sx={{
                        ...
                        border:
                          selectedQus?.answer === item && "1px solid green",
                      }}
...
function NestedList({ data, getQuestionDetail, activeSelect }) {
  ...
  return (
    <List
      sx={{ width: "98%", bgcolor: "background.paper" }}
      component="nav"
                    ...
                    <ListItemText
                      primary={
                        <span className="font-inter text-sm  text-gray-700">
                          {checkQuestionType(ques.question_type)}
                        </span>
                      }
                    />
                  </ListItemButton>
